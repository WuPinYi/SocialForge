...
type PostQuery struct {
	...
	withInfluencer *InfluencerQuery
...
// QueryInfluencer chains the current query on the "influencer" edge.
func (pq *PostQuery) QueryInfluencer() *InfluencerQuery {
	query := (&InfluencerClient{config: pq.config}).Query()
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withInfluencer: pq.withInfluencer.Clone(),
...
// WithInfluencer tells the query-builder to eager-load the nodes that are connected to
// the "influencer" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithInfluencer(opts ...func(*InfluencerQuery)) *PostQuery {
	query := (&InfluencerClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withInfluencer = query
	return pq
}
...
//
//	client.Post.Query().
//		GroupBy(post.FieldInfluencerID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withInfluencer; query != nil {
		if err := pq.loadInfluencer(ctx, query, nodes, nil,
			func(n *Post, e *Influencer) { n.Edges.Influencer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadInfluencer(ctx context.Context, query *InfluencerQuery, nodes []*Post, init func(*Post), assign func(*Post, *Influencer)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Post)
	for i := range nodes {
		fk := nodes[i].InfluencerID
	...
	query.Where(influencer.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "influencer_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PostQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withInfluencer != nil {
			_spec.Node.AddColumnOnce(post.FieldInfluencerID)
		}
	}
